  ...
  return (
    ...
    >
      <div className="flex-1 overflow-hidden flex flex-col md:flex-row min-h-0">
        {/* Левая колонка: список котов (на мобильных горизонтальный скролл) */}
        <div className="w-full md:w-52 bg-black/25 border-b md:border-b-0 md:border-r border-white/5 p-3 flex flex-row md:flex-col justify-between shrink-0 max-md:max-h-[120px] md:h-auto overflow-x-auto md:overflow-y-auto">
          <div className="min-h-0 flex flex-row md:flex-col gap-1.5 pb-1">
            {profile.cats.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`p-2 rounded-xl border transition-all cursor-pointer flex items-center gap-2 text-left shrink-0 min-w-[100px] md:min-w-full ${
                    isSelected
          ...
          <button
            ...
            className="w-full md:w-full p-2.5 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-600 hover:to-indigo-700 text-white text-[11px] font-bold flex items-center justify-center gap-1.5 transition-all active:scale-95 cursor-pointer border border-sky-400/10 shadow-lg shadow-sky-500/10 mt-2 md:mt-0 shrink-0"
          >
            <PlusCircle size={12} />
            <span>Приютить</span>
          </button>
        ...
        <div className="flex-1 flex flex-col overflow-y-auto relative p-3 md:p-5 bg-slate-950/40">
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-2.5 mb-4 shrink-0">
            <div className="flex gap-1.5 p-1 bg-black/35 rounded-xl border border-white/5 flex-wrap">
              <button
          ...
          {rightTab === 'care' && (
            <>
              {/* Центральная часть: кот, информация, кухня */}
              <div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-4 md:gap-6 relative min-h-[200px]">
                {/* Сцена с котом */}
                <div
                  ref={catStageRef}
                  id="cat-stage"
                  ...
                  className="relative bg-white/5 hover:bg-white/10 active:scale-98 transition-all border border-white/5 rounded-2xl w-32 h-32 md:w-44 md:h-44 flex flex-col items-center justify-center shadow-inner overflow-hidden cursor-pointer group select-none touch-none"
              ...
              >
                ...
                <div className="scale-95 md:scale-105 pointer-events-none select-none">
                  <CatRenderer
                    ...
                    accessory={(activeCat as any).accessory}
                    status={activeCat.status}
                    size={window.innerWidth < 768 ? 150 : 140}
                    personality={activeCat.personality}
                  />
                </div>
                ...
                {activeCat.status !== 'sleeping' && !isGroomingComplete && (
                  <motion.div
                    ...
                    className="absolute w-12 h-12 rounded-full bg-slate-800 border-2 border-sky-400 shadow-2xl flex items-center justify-center text-[10px] font-bold text-sky-400 cursor-grab active:cursor-grabbing hover:bg-slate-700 hover:scale-105 transition-colors z-40 active:scale-95 select-none"
                    style={{ x: 0, y: -40 }}
                  >
                    ЩЁТКА
                  </motion.div>
